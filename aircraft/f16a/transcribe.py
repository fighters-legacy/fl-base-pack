...
def checks():
    ...
    cm0 = cm_total(0.0, 0)
    print(f"""
  DROPPED (no schema slot), quantified:
    Cm0 at the model CG        = {cm0:+.4f}   (zero-elevator trim alpha shift ~ {-cm0 / m['cm_alpha'] / DEG:+.1f} deg)
    Ixz                        = {IXZ:.0f} kg m^2 (report Table I)
    CXq / CZq                  = typed, alpha-dependent (see tables above)
    Cnp / Clr                  = typed, alpha-dependent
    adverse yaw cn_da          = {cn_da:+.4f}/rad
    rudder roll cl_dr          = {cl_dr:+.4f}/rad
    dCl_beta / dCn_beta corr   = negligible (<= 0.001 at 3 alphas)
    dCm,sb / dCZ,sb            = typed; schema has speedbrake_cd only
    T_idle deck                = published, incl. negative ram-drag cells; engine is linear
    engine angular momentum He = 216.9 kg m^2/s (report p. 40)
